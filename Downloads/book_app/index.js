...
fetch(`https://www.googleapis.com/books/v1/volumes?q=${title}+inauthor:${author}&${key}&maxResults=5`)
  .then(response => response.json())
  .then(data => {
    response = data.items.map(e=> `{
        title:${!e.volumeInfo.title ? "No title found" : e.volumeInfo.title},
        author:${!e.volumeInfo.authors ? "No Author found" : e.volumeInfo.authors},
        company:${!e.volumeInfo.publisher ? "Publisher not found" : e.volumeInfo.publisher},
   }`);


   console.log(response.map(e=> `Book ID :${response.indexOf(e)} ${e}`));
   prompt("enter the book id for the book you'd like to save to your reading list. (Click Enter)")
  ...
  fs.readFile("database.json","utf8",function (err,data){
      ...
      fs.writeFileSync("database.json", JSON.stringify(obj.users));
